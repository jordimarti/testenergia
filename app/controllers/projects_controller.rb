class ProjectsController < ApplicationController
  ...
  def print
    ...
    render layout: "autotest_print_layout"
  end
  ...
  def update_personal_info
    @project = Project.find(params[:id])
    if @project.update_attributes(project_params)
      redirect_to(:action => 'contract', :id => @project.id)
    else
      render('infotest')
    end
  end
  ...
  def contract
    @project = Project.find(params[:id])
    #Find the last xproject to assign the next id to the new xproject
    last_xproject = Xproject.last
    @project.id = last_xproject.id + 1
